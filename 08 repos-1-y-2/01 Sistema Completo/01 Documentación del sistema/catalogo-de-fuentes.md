...
Cada archivo `XXb-decisiones-tomadas-y-fuentes.md` usa **8 columnas**:
...
- **Viene de una decisión anterior** — `no` si el valor nace en esta fila; `← X.YY` (o `← X.YY, Z.ZZ`) si cascadea de filas previas.
- **Referencia exacta en el manual** — referencia canónica `§X` del documento ejecutable del paso (`XXa-ejecucion-...md`).
...
la columna Viene de una decisión anterior debe señalar la fila origen con `← X.YY`.
Si combina varias filas: `← X.YY, Z.ZZ`. Si es un rango: `← X.YY–X.ZZ`.
```

### Regla 4 — `Por qué esta decisión` no repite la Decisión
...
Cerradas:    Fuente ∈ {GMB Crush solo, Input humano, Decisión de diseño}
             Y Viene de una decisión anterior apunta a fila ya cerrada (o `no`).

Pendientes:  Fuente contiene `Competidores`, `Datos de búsqueda` o `IA sin respaldo`.
             O Viene de una decisión anterior apunta a fila pendiente.
```
...
- 0 filas superan las 12 palabras en `Por qué esta decisión`. Media: 8.6 palabras.
